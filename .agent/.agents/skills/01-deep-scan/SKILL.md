...
5. **Anomaly Detection**: Flag any structural non-compliance — missing `assets/` taxonomy, absent root **`AETHER.md`**, stray reference folders outside `archived/`. **Do NOT flag the `.agent/` folder as an anomaly — it is expected and correct.**
