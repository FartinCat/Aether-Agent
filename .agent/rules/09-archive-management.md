...
1. **Read-Only Inspiration**: Treat any folder named `frontend[x]`, `app[x]`, or similar iterative backups as an archive candidate. These folders contain older iterations and discarded resources. They are strictly **Read-Only** sources of inspiration and previous design patterns. They MUST NEVER be linked to, imported by, or modified during active code generation for the live application.
...
4. **Execution**: Create an `archived/` folder if it doesn't exist. Move all identified stray reference folders into `archived/`. Announce what was moved and what was skipped (protected).
