...
If you (the Agent) have modified the codebase in a way that alters logic, adds features, or fixes bugs during the current session, you **MUST** ensure the version is bumped before finalizing the work via **`/17-auto-commit`**.
...
### Execution Trigger
This rule is automatically enforced by the **`sync_registry.py`** engine during **`/16-sync-registry`**.
1. The script runs `git status` / `git diff`.
2. It evaluates the changes against the criteria above.
3. It autonomously bumps **`AETHER.md`** (version fields) before syncing README/LICENSE/install-state.
