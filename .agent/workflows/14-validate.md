...
- `scanner` / `build-website` / `build-app` → source files exist, tests exist, root **`AETHER.md`** version was bumped (§1 / §14)

- `fix-bugs` → patches were applied, version was bumped in **`AETHER.md`**
...
Read **`AETHER.md`** title line and identity table version; confirm they match **§14 Project Metadata** `**Version**:`.
If they differ:
```
SELF-CHECK FAIL: AETHER.md internal version drift — run /sync-registry
```

#### 5c — Hardcoded Version Detection
Scan all `.agent/workflows/*.md` files for literal version strings like `v3.0.0`,
`v4.0.0`, etc. that should be dynamically read from **`AETHER.md` §14**. Flag each:
```
