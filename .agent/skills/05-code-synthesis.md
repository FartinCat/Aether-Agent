...
**Synthesis Rule**: If Plan A (Claude) and Plan B (DeepSeek) conflict on a database choice, the Synthesizer must evaluate the choice against `04-architectural-design.md` mandates first. If still tied, the model with the higher weight for that specific domain wins.
...
2. **Foundational Mandates**: Does the choice violate `04-architectural-design.md` or `01-core.md`? (Precedence: 90%).
